...
m1<-pivot_longer(metrics1, cols=starts_with ("P"))
m2<-as.data.frame(m1)
...
# glm for evenness/h2 (add/remove variables from model2 to conduct nested model comparison)
model1<-glm(Evenness~habitat+lat+richness+log(matrixsize)+family+lat*family+habitat*family, family = gaussian , data=evenness1)
model2<-glm(Evenness~lat+richness+log(matrixsize), family = gaussian , data=evenness1)

model1<-glm(H2prime~habitat+lat+richness+log(matrixsize)family+lat*family+habitat*family, family = gaussian , data=h2prime)
...
entirefullweb<-read.csv(file= "entireweb.csv", header=T, stringsAsFactors = FALSE)




farmlandweb<- frame2webs(farmlandalldata, varnames = c("host_species", "diptera_species", "webID", "frequency"))
urbanweb<- frame2webs(urbanalldata, varnames = c("host_species", "diptera_species", "webID", "frequency"))
seminaturalweb<- frame2webs(seminaturalalldata, varnames = c("host_species", "diptera_species", "webID", "frequency"))
entirefullweb1<- frame2webs(entirefullweb, varnames = c("host_species", "diptera_species", "webID", "frequency"))



interactioncolour4<-c(rep("#2c7bb6",51),rep("#d7191c",110), rep("#ffffbf",7), rep("#abd9e9", 17), rep("#fdae61", 18 ))

plotweb(entirefullweb1$Pall,text.rot = 90, high.lablength = 0, low.lablength = 0, method = "normal", col.high = interactioncolour4, col.interaction = interactioncolour4,
        bor.col.interaction=interactioncolour4, bor.col.high=interactioncolour4)



interactioncolour1<-c(rep("#2c7bb6",29),rep("#d7191c",26), rep("#ffffbf",2), rep("#abd9e9", 8))


plotweb(farmlandweb$Pall, text.rot = 90, high.lablength = 0, low.lablength = 0, method = "normal", col.high = interactioncolour1, col.interaction = interactioncolour1,
        bor.col.interaction=interactioncolour1, bor.col.high=interactioncolour1) 



interactioncolour2<-c(rep("#2c7bb6",37),rep("#d7191c",55), rep("#ffffbf",2), rep("#abd9e9", 3), rep("#fdae61", 18 ))

plotweb(seminaturalweb$Pall, text.rot = 90,
        x.lim = c(0.22,6.5), high.lablength = 0, low.lablength = 0, method = "normal", col.high = interactioncolour2, col.interaction = interactioncolour2,
        bor.col.interaction=interactioncolour2, bor.col.high=interactioncolour2)



interactioncolour3<-c(rep("#d7191c",59), rep("#ffffbf",5), rep("#abd9e9", 10))

plotweb(urbanweb$Pall, text.rot = 90,high.lablength = 0, low.lablength = 0, method = "normal", col.high = interactioncolour3, col.interaction = interactioncolour3,
        bor.col.interaction=interactioncolour3, bor.col.high=interactioncolour3)
